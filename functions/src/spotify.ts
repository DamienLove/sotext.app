...
export const getSpotifyAccessToken = onCall({secrets: [spotifyClientId, spotifyClientSecret]}, async (request) => {
  // 1. Authenticate the user
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "User must be authenticated.");
  }

  // 2. Retrieve secrets using defineSecret() instead of process.env for better security.
