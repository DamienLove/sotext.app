# SoText Troubleshooting Guide

## 🔔 Notifications & Do Not Disturb (DND)

### Issue: My contact didn't hear the alert!
**Solution:**
1.  **Check Permissions**: Ensure the *recipient* has granted SoText "Do Not Disturb Access".
2.  **Android 15+ Users**: Due to system changes, you must manually allow "Bypass DND" for the "Emergency Alerts" notification channel.
    *   Go to **Settings > Apps > SoText > Notifications**.
    *   Tap **Emergency Alerts**.
    *   Toggle **Override Do Not Disturb** to **ON**.
3.  **Check Volume**: SoText attempts to maximize volume, but ensure the device isn't connected to silent Bluetooth headphones.

### Issue: "Partial Override" Warning
**Solution:**
This means SoText raised the volume but couldn't disable the global DND setting. This is common on newer Android versions. As long as the **Notification Channel Bypass** is enabled (see above), the alert will still sound.

## 📡 Connectivity & Alerts

### Issue: Messages aren't sending via Data/WiFi.
**Solution:**
SoText uses a "Firebase-First" system. If data fails:
...
2.  **Background Restrictions**: Ensure SoText is excluded from "Battery Optimization" so the listening service isn't killed by the system.
...
**Solution:**
Web access is a Premium feature. Ensure your subscription is active, then visit `https://sotext.app` and scan the QR code from the mobile app settings.
