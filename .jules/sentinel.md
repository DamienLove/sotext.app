...
## 2024-05-24 - Account Deletion IDOR via Trusting Modifiable User Fields
**Vulnerability:** IDOR vulnerability in `deleteAccount` Cloud Function where deletion of device documents and `betaAgreements` documents relied on the `deviceId` field read directly from the user's modifiable profile document. A malicious user could modify their profile's `deviceId` field to point to an arbitrary target device ID and then delete their account, causing the backend to erroneously delete the targeted victim's device document and beta agreement.
**Learning:** Never trust document reference fields or IDs stored inside user-modifiable documents (like profiles) to perform sensitive operations. The backend inadvertently trusted user data for an operation affecting other collections.
**Prevention:** Always rely on server-authoritative queries matching the authenticated user's ID (e.g., querying `devices` where `uid` == `auth.uid`) to collect IDs to act upon, rather than blindly fetching and trusting IDs from the user's profile document.
