...
## 2024-05-18 - Use defineSecret() for Firebase Functions Environment Variables
**Vulnerability:** External credentials (like SMTP passwords and API secrets) were hardcoded in environment variables accessed via process.env.
**Learning:** In Firebase Functions, `defineSecret` from `firebase-functions/params` provides better security by integrating with Google Cloud Secret Manager instead of relying on plain environment variables. However, external services that need these secrets (e.g., `nodemailer.createTransport()`) must be lazily initialized inside the function handler itself since `.value()` can only be accessed during runtime execution, not at module load time.
**Prevention:** Always use `defineSecret()` for sensitive variables in Firebase Functions and pass them into the `onCall` (v2) or `.runWith({secrets: [...]})` (v1) configuration. Ensure any third-party dependencies needing these secrets are instantiated inside the function handler.
