...
## 2024-05-28 - Insecure Secret Management
**Vulnerability:** The `getSpotifyAccessToken` Cloud Function previously relied on `process.env` to load sensitive API credentials (`SPOTIFY_CLIENT_ID` and `SPOTIFY_CLIENT_SECRET`). Storing secrets in standard environment variables increases the risk of exposure via accidental logging, misconfigured debug endpoints, or unauthorized access to the execution environment.
**Learning:** Environment variables are often persisted in plaintext and easily accessible by any process in the environment. Firebase Secret Manager provides a more secure approach by injecting secrets only when explicitly requested at runtime and securely encrypting them at rest.
**Prevention:** Always use Firebase's `defineSecret` mechanism (from `firebase-functions/params`) to manage and inject sensitive credentials into Cloud Functions. Declare required secrets in the function configuration (e.g., `{ secrets: [spotifyClientId] }`) to enforce least-privilege access.
