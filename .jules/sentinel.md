...
## 2024-05-28 - Insecure Access to Credentials via process.env
**Vulnerability:** Sensitive credentials like email account passwords and Spotify client secrets were being loaded directly from `process.env`. In Firebase, environment variables are often stored insecurely or exposed in deployment configurations.
**Learning:** `process.env` is not a secure way to manage sensitive secrets like API keys, client secrets, or database passwords in Firebase Functions. `defineSecret()` from `firebase-functions/params` should always be used as it integrates securely with Google Cloud Secret Manager. Furthermore, when using `defineSecret()`, secret values (accessed via `.value()`) are only available inside the function handler execution context, so clients or external services relying on them (like nodemailer transporters) must be lazily initialized.
**Prevention:** Always use `defineSecret()` to declare and access secrets. Wrap external service initializations in a getter or lazy-init function, and remember to bind the secrets to the function using `.runWith({secrets: [...]})` for v1 or passing the `{secrets: [...]}` option for v2 functions.
