...
## 2024-05-27 - Hardcoded Secrets via process.env
**Vulnerability:** Environment variables (`process.env.SPOTIFY_CLIENT_SECRET`, `process.env.EMAIL_PASS`) were used to handle sensitive credentials in Cloud Functions. This lacks built-in security features, potentially exposing secrets if environment configurations are dumped or mismanaged.
**Learning:** `process.env` is not secure for sensitive keys in Firebase Functions. Firebase provides `defineSecret()` from `firebase-functions/params`, which securely fetches Google Cloud Secret Manager values. However, functions using `defineSecret()` must have the secret explicitly passed in the function config (e.g., `onCall({secrets: [mySecret]})` or `.runWith({secrets: [mySecret]})`), and external initializations relying on these secrets (like nodemailer `createTransport`) MUST occur lazily inside the function handler, as secret values are not populated during global module initialization.
**Prevention:** Always use `defineSecret()` for passwords and API keys, and `defineString()` for non-sensitive IDs. Ensure libraries requiring secrets are instantiated within the function execution scope, not globally.
