...
## 2024-05-28 - Insecure Secret Management via Process Environment
**Vulnerability:** Application secrets (like email passwords or Spotify Client Secrets) were being loaded directly from `process.env` at module load time. This exposes secrets to the entire Node.js runtime environment and logging mechanisms inadvertently, and causes deployment failures if the secret is strictly managed or rotated dynamically, as they are evaluated when the code boots.
**Learning:** `process.env` does not guarantee isolation and leaks easily into crash dumps or dependencies. Furthermore, Cloud Functions v2 requires explicit bindings for secrets so they can be rotated and audited via Google Cloud Secret Manager. Evaluating secrets at module load also prevents proper lazy-loading.
**Prevention:** Always use `defineSecret()` from `firebase-functions/params` and explicitly bind them using `.runWith({secrets: [...]})` (v1) or the `secrets` array option in v2 `onCall`. Access the value lazily inside the function handler using `.value()` to ensure it's not leaked during module initialization.
