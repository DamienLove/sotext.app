...
**Vulnerability:** Secrets like `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `EMAIL_USER`, and `EMAIL_PASS` were accessed directly via `process.env` in Firebase Functions, risking exposure in logs, error messages, and making local development/deployment insecure.
**Learning:** `process.env` relies on environment variables which are difficult to manage securely across environments (dev, staging, prod) and are often plain text. Using `defineSecret()` from `firebase-functions/params` tightly integrates with Google Cloud Secret Manager, ensuring secrets are encrypted at rest, injected only at runtime, and easily rotatable.
**Prevention:** Always use `defineSecret()` for handling sensitive information (API keys, passwords, tokens) in Firebase Functions. Pass the defined secrets to the function configuration using `.runWith({secrets: [...]})` for v1 functions or `onCall({secrets: [...]})` for v2 functions, and retrieve their values securely at runtime using `.value()`.
