...
## 2024-05-28 - Insecure Secret Handling via process.env
**Vulnerability:** Sensitive credentials (like `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, `EMAIL_USER`, and `EMAIL_PASS`) were being fetched directly from `process.env` in Cloud Functions. This is insecure as environment variables can be easily leaked or misconfigured, and they don't provide the secure, encrypted storage mechanism that Firebase Secret Manager offers.
**Learning:** Firebase's `defineSecret` from `firebase-functions/params` should always be used instead of `process.env` for sensitive data. Critically, when using `defineSecret`, clients or external services (like a `nodemailer` transporter) must be initialized lazily *inside* the function handler. This is because secret values (accessed via `.value()`) are not available at module load time; they are only populated when the function executes.
**Prevention:** Always use `defineSecret` to manage secrets. Ensure that any service initialization requiring these secrets occurs within the function body or is lazily loaded upon the first function invocation, and remember to pass the secrets into the function configuration (`.runWith({secrets: [...]})` for v1 or `{secrets: [...]}` for v2).
