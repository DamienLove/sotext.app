...
## 2025-04-02 - [Secrets Exfiltration Risk via Environment Variables]
**Vulnerability:** Cloud functions (`email.ts`, `spotify.ts`) used `process.env` to load sensitive credentials. This bypasses Cloud Secret Manager, risking accidental exposure in deployment configs, logs, or if the environment variables are dumped. Additionally, module-level initialization meant secrets needed to be available immediately upon function deployment/start.
**Learning:** `defineSecret()` is required in Firebase Functions to securely inject secrets from Google Cloud Secret Manager. However, `defineSecret()` returns a `SecretParam` object whose `.value()` is only available *inside* the function handler when it runs, not during module initialization.
**Prevention:** Always use `defineSecret()` for credentials and pass them in the function configuration (`.runWith({secrets: [...]})` for v1 or `{secrets: [...]}` for v2). Crucially, any clients using these secrets (like a nodemailer transporter or an API client) MUST be lazily initialized inside the function handler to successfully access `.value()`.
