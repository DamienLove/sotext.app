...
## 2026-04-24 - Hardcoded Secrets in Source Code
**Vulnerability:** The Spotify integration () used `process.env` to load credentials. While better than plain text strings, it implies reliance on  files or basic environment variables which can easily be misconfigured or leaked in build logs or deployment artifacts, especially in serverless environments.
**Learning:** For serverless deployments like Firebase Functions, using the platform's native secret manager (`defineSecret`) ensures that secrets are encrypted at rest, securely injected only at runtime, and never exposed in the function's deployment bundle or standard environment variable dumps.
**Prevention:** Always use `defineSecret` from `firebase-functions/params` for sensitive credentials (API keys, client secrets) instead of relying on `process.env`. Ensure secrets are explicitly bound to the functions that need them via the `secrets` array in the function configuration.
## 2024-05-28 - Hardcoded Secrets in Source Code
**Vulnerability:** The Spotify integration (`spotify.ts`) used `process.env` to load credentials. While better than plain text strings, it implies reliance on `.env` files or basic environment variables which can easily be misconfigured or leaked in build logs or deployment artifacts, especially in serverless environments.
**Learning:** For serverless deployments like Firebase Functions, using the platform's native secret manager (`defineSecret`) ensures that secrets are encrypted at rest, securely injected only at runtime, and never exposed in the function's deployment bundle or standard environment variable dumps.
**Prevention:** Always use `defineSecret` from `firebase-functions/params` for sensitive credentials (API keys, client secrets) instead of relying on `process.env`. Ensure secrets are explicitly bound to the functions that need them via the `secrets` array in the function configuration.
