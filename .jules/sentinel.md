...
## 2026-05-12 - IDOR in Account Deletion via Modifiable Profile Fields
**Vulnerability:** The `deleteAccount` Cloud Function relied on the `deviceId` field stored in the user's modifiable profile (`users/{uid}`) to delete device registrations and beta agreements. A malicious user could update this field to a victim's device ID and trigger account deletion, leading to an Insecure Direct Object Reference (IDOR) and arbitrary deletion of other users' devices.
**Learning:** Client-modifiable fields must never be used to authorize destructive operations on other collections. Backend functions execute with elevated privileges (admin) and bypass Firestore rules, making them prime vectors for IDOR if inputs are not strictly validated or derived authoritatively.
**Prevention:** When performing sensitive document deletions or updates (like removing associated user data), always rely on server-authoritative queries matching the authenticated user's ID (e.g., `where("uid", "==", uid)`) rather than trusting foreign keys or reference fields stored within the user's modifiable profile document.
