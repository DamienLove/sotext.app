...
## 2026-03-19 - Replace process.env with defineSecret for Secrets Management
**Vulnerability:** Firebase Functions relying on process.env to retrieve sensitive credentials (e.g. API keys, emails, passwords) risk exposing them through dependency leaks, logging misconfigurations, or environment variable inspection.
**Learning:** Hardcoding secrets or managing them via environment variables in `process.env` does not leverage Google Cloud Secret Manager. The `defineSecret` mechanism ensures access is secure, auditable, and restricts access strictly to the functions explicitly requesting it via `.runWith({secrets: [...]})` or `onCall({secrets: [...]})`.
**Prevention:** Avoid relying on `process.env` for accessing sensitive parameters. Always use `defineSecret` from `firebase-functions/params` and explicitly pass required secrets into the function configurations.
