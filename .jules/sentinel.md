...
## 2025-02-28 - [Insecure Secret Management via process.env]
**Vulnerability:** Hardcoded environment variables were being used for sensitive values like API keys and passwords via `process.env`. While sometimes necessary, relying on `process.env` directly makes rotating or auditing secrets more difficult, and increases the likelihood of exposing them via unintended logs or deployment configurations.
**Learning:** `defineSecret` from `firebase-functions/params` provides better auditing, allows secrets to be tied to Cloud Secret Manager directly, and prevents secrets from being accidentally exported if the overall environment configuration is dumped. A key learning is that any service that needs these secrets at initialization time (like a nodemailer transporter) must be lazily initialized inside the Cloud Function execution scope, as `.value()` is only populated *during* function execution.
**Prevention:** Avoid `process.env` for all sensitive credentials. Use Firebase Genkit's `defineSecret` and always initialize third-party clients lazily within the function execution context so the secrets are actually available when needed.
