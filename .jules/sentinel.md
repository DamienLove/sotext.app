...
## 2026-04-15 - Hardcoded Secrets / process.env in Firebase Functions
**Vulnerability:** Retrieving sensitive credentials like `SPOTIFY_CLIENT_SECRET` directly from `process.env` risks accidental exposure during debugging or error reporting, and makes it difficult to audit secret usage.
**Learning:** The `defineSecret` API from `firebase-functions/params` is required to handle sensitive data securely in Cloud Functions. Accessing secret values at the top level or via `process.env` is insecure and bypasses Firebase's integrated secret management.
**Prevention:** Use `defineSecret` for all sensitive data and access its value using `.value()` only inside the function handler. Non-sensitive configuration values should use `defineString`. Pass secrets explicitly to the function configuration using `secrets: [mySecret]`.
