...
## 2024-04-21 - Secure Firebase Secrets via defineSecret
**Vulnerability:** Hardcoded environment variables (`process.env.SECRET_NAME`) used in production function files (e.g. `email.ts` and `spotify.ts`), which leak secrets into module-level variables during deployment or execution, rather than using Firebase's modern secure configuration management.
**Learning:** External services like `nodemailer` transporter require their clients to be initialized safely during the function call (lazy initialization) rather than at the top level of the module when accessing values from `defineSecret("...").value()`, as the secrets are only securely populated at function runtime.
**Prevention:** Always use `defineSecret()` from `firebase-functions/params` rather than `process.env`. When integrating third-party clients that need secret variables for authentication, initialize them lazily inside the handler. Inject the secrets in the function signature by using `.runWith({secrets: [...]})` for v1 or `onCall({secrets: [...]}, ...)` for v2.
