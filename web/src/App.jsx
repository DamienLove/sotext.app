...
function App() {
  ...
  const combinedThreads = useMemo(() => {
    ...
    // Bolt: Use imperative loops instead of chained flat(), map(), and filter()
    // to prevent intermediate array allocations when computing the combined thread list.
    const lineFlattened = [];
    const lineAddresses = new Set();

    for (const threadsArray of Object.values(lineThreads)) {
      if (!Array.isArray(threadsArray)) {
         lineFlattened.push(threadsArray);
         if (threadsArray && threadsArray.address) {
             lineAddresses.add(threadsArray.address);
         }
         continue;
      }
