...
import com.sotext.data.sms.SmsCodec
...
import com.sotext.util.resolveUri
import com.sotext.ui.EmergencyPopupActivity
...
class ContactLinkManager @Inject constructor(
    ...
    private suspend fun sendEmailNotification(contact: Contact, message: PulseLinkMessage): Boolean {
        ...
        val data = hashMapOf(
            ...
            "senderName" to (auth.currentUser?.displayName ?: "SoText User"),
    ...
    private fun convertPayloadToMessage(payload: LinkChannelPayload): PulseLinkMessage? {
        ...
        return when (payload.type) {
            ...
            "manual" -> {
                val urgency = runCatching { com.sotext.domain.model.MessageUrgency.valueOf(getStr("urgency").orEmpty()) }.getOrDefault(com.sotext.domain.model.MessageUrgency.STANDARD)
                val volume = getStr("volumeHint")?.let { runCatching { com.sotext.domain.model.VolumeHint.valueOf(it) }.getOrNull() }
    ...
    private suspend fun deliverManualMessage(
        ...
        urgency: com.sotext.domain.model.MessageUrgency,
        volumeHint: com.sotext.domain.model.VolumeHint?
    ) {
        ...
        val tier = when (urgency) {
            com.sotext.domain.model.MessageUrgency.EMERGENCY,
            com.sotext.domain.model.MessageUrgency.URGENT -> EscalationTier.EMERGENCY
            com.sotext.domain.model.MessageUrgency.STANDARD -> EscalationTier.CHECK_IN
        }
        remoteActionHandler.playAttentionTone(
            contact = contact,
            tier = tier,
    ...
    suspend fun cancelActiveEmergency(): Boolean = withContext(Dispatchers.IO) {
        ...
        contacts.forEach { contact ->
            ...
            val message = PulseLinkMessage.ManualMessage(
                ...
                code,
                body,
                com.sotext.domain.model.MessageUrgency.STANDARD,
                null
            )
