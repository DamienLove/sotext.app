package com.sotext.data.settings
...
import com.sotext.domain.model.AlertProfile
...
class SettingsRepositoryImpl @Inject constructor(
    ...
    override val settings: Flow<PulseLinkSettings> = dataStore.data.map { prefs ->
        ...
        PulseLinkSettings(
            ...
            timeFormat = prefs[TIME_FORMAT]?.let { runCatching { com.sotext.domain.model.TimeFormat.valueOf(it) }.getOrNull() }
                ?: PulseLinkSettings().timeFormat,
            themePreferences = decodeJsonOrNull(prefs[THEME_PREFERENCES]) {
                json.decodeFromString(com.sotext.domain.model.ThemePreferences.serializer(), it)
    ...
    override suspend fun update(transform: (PulseLinkSettings) -> PulseLinkSettings) {
        editOnIo { prefs ->
            ...
            prefs[THEME_PREFERENCES] = encodeJson {
                json.encodeToString(
                    com.sotext.domain.model.ThemePreferences.serializer(),
    ...
    override suspend fun setTimeFormat(format: com.sotext.domain.model.TimeFormat) {
        editOnIo { prefs ->
            prefs[TIME_FORMAT] = format.name
        }
    }

    override suspend fun setThemePreferences(theme: com.sotext.domain.model.ThemePreferences) {
        editOnIo { prefs ->
            prefs[THEME_PREFERENCES] = encodeJson {
                json.encodeToString(com.sotext.domain.model.ThemePreferences.serializer(), theme)
            }
        }
    }
